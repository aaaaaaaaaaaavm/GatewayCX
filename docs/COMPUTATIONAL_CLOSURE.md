...
This register prevents a passing test suite from being mistaken for a closed architecture.

| Question | Model or test | Present closure | Missing evidence |
|---|---|---|---|
| What is geometric Earth–Moon light time? | Exact deterministic equation and S006 range | Closed for cited closest, mean and farthest inputs | Time-varying operational ephemeris |
...
| Where should IP terminate and DTN begin? | ADR-0002 + S005 + S030 external wire-interoperability harness | One exact dtn7-go → bp7-rs crossing through gateway/ledger/auth in run 33294426808; reciprocal parser incompatibility recorded | Bidirectional external wire acceptance, full BP daemons, convergence layers, BPSec, contact routing and application policy |
...
| Can regional identity, consistency and updates fail safely? | S028 X07/X08/X14 executable files, signatures and SQLite faults | Closed for bounded reference mechanisms | Production PKI, distributed database, registry, secure boot and service traces |
| Can a software black start execute without Earth? | S028 X16 child-process launch and time fault | Closed for five synthetic processes | Electrical power, real daemons, oscillator behaviour and network interfaces |
...
| Does recovery survive varied transaction faults and detected file corruption? | S028 X18 seeded 60-transaction replay and backup restore | Closed for local SQLite reference fault points | Raw block device, cache, filesystem and multi-node recovery |
